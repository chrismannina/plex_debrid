...
def logerror(response):
    ...
    if response.status_code == 401:
        ui_print(
            "[debridlink] error 401: debridlink api key does not seem to work. check your debridlink settings."
        )
...
def get(url):
    ...
    try:
        ...
        ui_print(
            "debridlink error: (json exception): " + str(e), debug=ui_settings.debug
        )
        response = None
    return response
...
def oauth(code=""):
    if code == "":
        response = post(
            "https://debrid-link.fr/api/oauth/device/code", "client_id=" + client_id
        )
    ...
    for release in cached[:]:
        ...
        if regex.match(r"(" + query + ")", release.title, regex.I) or force:
            if stream:
                ...
                hashstring = regex.findall(
                    r"(?<=btih:).*?(?=&)", str(release.download[0]), regex.I
                )[0]
                ...
                try:
                    if hasattr(response.value, hashstring.lower()):
                        ...
                        if response.success:
                            ui_print(
                                "[debridlink] adding cached release: " + release.title
                            )
                            return True
                except:
                    continue
                ...
                try:
                    ...
                    if response.success:
                        ui_print(
                            "[debridlink] adding uncached release: " + release.title
                        )
                        return True
                except:
                    continue
