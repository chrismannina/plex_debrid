    ...
    if "error" in str(response.content):
        try:
            ...
            ui_print(
                ...
                + str(response.status_code)
                + ": "
                + response2.data[0].error.message
            )
        except:
            try:
                response2 = json.loads(
                ...
                ui_print(
                    "[alldebrid] error " + str(response.status_code) + ": unknown error"
                )
    if response.status_code == 401:
        ui_print(
            "[alldebrid] error: 401: alldebrid api key does not seem to work. check your alldebrid settings."
        )
...
def get(url):
    ...
    try:
        ...
        ui_print(
            "[alldebrid] error: (json exception): " + str(e), debug=ui_settings.debug
        )
        response = None
    return response
    ...
    for release in cached[:]:
        ...
        if regex.match(r"(" + query + ")", release.title, regex.I) or force:
            if stream:
                ...
                url = (
                    "https://api.alldebrid.com/v4/magnet/instant?magnets[]="
                    + release.download[0]
                )
                response = get(url)
                instant = False
                ...
                if instant:
                    url = (
                        "https://api.alldebrid.com/v4/magnet/upload?magnets[]="
                        + release.download[0]
                    )
                    response = get(url)
                    torrent_id = response.data.magnets[0].id
                    url = "https://api.alldebrid.com/v4/magnet/status?id=" + str(
                        torrent_id
                    )
                    response = get(url)
                    ...
                    if len(torrent_links) > 0:
                        ...
                        for link in torrent_links:
                            url = (
                                "https://api.alldebrid.com/v4/link/unlock?link="
                        ...
                        if success:
                            ...
                            url = (
                                "https://api.alldebrid.com/v4/user/links/save?links[]="
                                + saved_links
                            )
                            response = get(url)
                            ui_print(
                                "[alldebrid] adding cached release: " + release.title
                            )
                            return True
                        else:
