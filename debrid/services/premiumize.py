...
def logerror(response):
    if not response.status_code == 200:
        ui_print(
            "[premiumize] error: " + str(response.content), debug=ui_settings.debug
        )
...
def get(url):
    ...
    try:
        ...
        ui_print(
            "[premiumize] error: (json exception): " + str(e), debug=ui_settings.debug
        )
        response = None
    return response
...
def post(url, data):
    ...
    try:
        response = session.post(
            url + "?apikey=" + api_key + data, headers=headers, data={}
        )
        logerror(response)
        response = json.loads(
            response.content, object_hook=lambda d: SimpleNamespace(**d)
    ...
    for release in cached[:]:
        ...
        if regex.match(r"(" + query + ")", release.title, regex.I) or force:
            if stream:
                ...
                url = (
                    "https://www.premiumize.me/api/cache/check?items[]="
                    + release.download[0]
                )
                response = get(url)
