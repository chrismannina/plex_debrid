    ...
    if "rarbg" in active:
        ...
        while not hasattr(response, "torrent_results") and retries < 5:
            if regex.search(r"(tt[0-9]+)", query, regex.I):
                url = (
                    "https://torrentapi.org/pubapi_v2.php?mode=search&search_imdb="
                    + str(query)
                    + "&ranked=0&category=52;51;50;49;48;46;45;44;41;17;14&token="
                    + token
                    + "&limit=100&format=json_extended&app_id=fuckshit"
                )
            else:
                url = (
                    "https://torrentapi.org/pubapi_v2.php?mode=search&search_string="
                    + str(query)
                    + "&ranked=0&category=52;51;50;49;48;46;45;44;41;17;14&token="
                    + token
                    + "&limit=100&format=json_extended&app_id=fuckshit"
                    ...
                    if hasattr(response, "error"):
                        if "Invalid token" in response.error:
                            ...
                            if len(response.content) > 5:
                                ...
                                ui_print(
                                    "rarbg error: could not fetch new token",
                                    debug=ui_settings.debug,
                                )
