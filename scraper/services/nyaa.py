...
errors = [
    ...
    [
        400,
        " bad request) make sure your nyaa parameters are correct and that the used proxy resembles the nyaa.si website closely.",
    ],
    [403, " permission denied)"],
    [503, " service unavailable)"],
    [
        404,
        " wrong parameter) make sure your nyaa parameters are correct and that the used proxy resembles the nyaa.si website closely.",
    ],
    [
        429,
        " too many requests) nyaa is likely blocking your ip. please use a proxy: "
        + str(proxies),
    ],
]
...
def scrape(query, altquery):
    ...
    if "nyaa" in active:
        if regex.search(r"(?<=nyaa)(.*?)(?=\))", altquery, regex.I):
            query = (
                '"'
                + regex.search(r"(?<=nyaa)(.*?)(?=\))", altquery, regex.I)
                .group()
                .replace(".", " ")
                .replace("|", '"|"')
                + '"'
        ...
        try:
                ...
                list = [
                    {
                        ...
                        "size": i.find_all("td", {"class": "text-center"})[
                            1
                        ].text.replace("i", ""),
                    ...
                    for count, torrent in enumerate(list):
                        ...
                        if regex.match(
                            ...
                            if regex.search(r"([0-9]*?\.[0-9])(?= KB)", size, regex.I):
                                size = regex.search(
                                    r"([0-9]*?\.[0-9])(?= KB)", size, regex.I
                                ).group()
                                size = float(float(size) / 1000 / 1000)
                            elif regex.search(
                                r"([0-9]*?\.[0-9])(?= MB)", size, regex.I
                            ):
                                size = regex.search(
                                    r"([0-9]*?\.[0-9])(?= MB)", size, regex.I
                                ).group()
            ...
            if hasattr(response, "status_code") and response.status_code == 200:
                ui_print(
                    "[nyaa] error: proxy unable to be scraped. please choose another proxy that resembles the nyaa.si website exactly: "
                    + str(proxies)
                )
            elif response == None:
                ui_print(
                    "[nyaa] error: nyaa could not be reached using the current proxy."
                )
            else:
                ui_print(
                    "[nyaa] error: unknown error. turn on debug printing for more information."
                )
