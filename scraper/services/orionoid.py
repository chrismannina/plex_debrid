...
def oauth(code=""):
    if code == "":
        response = get(
            "https://api.orionoid.com?keyapp="
            + client_id
            + "&mode=user&action=authenticate"
        )
        ...
        while response == None:
            response = get(
                "https://api.orionoid.com?keyapp="
                + client_id
                + "&mode=user&action=authenticate&code="
                + code
            )
...
def scrape(query, altquery):
    ...
    if not hasattr(response, "data") or (
        hasattr(response, "data") and not hasattr(response.data, "streams")
    ):
        try:
    ...
    try:
        if hasattr(response.data, "movie"):
            match = (
                response.data.movie.meta.title
                + " "
                + str(response.data.movie.meta.year)
            )
        elif hasattr(response.data, "show"):
            match = (
                response.data.show.meta.title + " " + str(response.data.show.meta.year)
            )
        ui_print(
            "[orionoid] matched query: '"
            + query
            + "' to "
            + type
            + " '"
            + match
            + "' - found "
            + str(response.data.count.total)
            + " releases (total), retrieved "
            + str(response.data.count.retrieved),
            ui_settings.debug,
        )
    except:
        None
    for result in response.data.streams:
        try:
            ...
            size = (
                float(result.file.size) / 1000000000
                if not result.file.size == None
                else 0
            )
            links = result.links
