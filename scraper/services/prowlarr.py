...
def scrape(query, altquery):
    ...
    if "prowlarr" in active:
        ...
        try:
            ...
            ui_print(
                "[prowlarr] error: prowlarr request timed out. Reduce the number of prowlarr indexers or make sure they are healthy."
            )
            return []
        except:
            ui_print(
                "[prowlarr] error: prowlarr couldnt be reached. Make sure your prowlarr base url is correctly formatted (default: http://localhost:9696)."
            )
            return []
...
def resolve(result):
    ...
    try:
        ...
        ui_print(
            "[prowlarr] error: resolver couldnt get magnet/torrent for release: "
            + result.title,
            ui_settings.debug,
        )
        return scraped_releases
