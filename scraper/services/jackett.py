...
def scrape(query, altquery):
    ...
    if "jackett" in active:
        ...
        try:
            ...
            ui_print(
                '[jackett] error: jackett request timed out. Reduce the number of jackett indexers, make sure your indexers are healthy and enable the jackett setting "CORS".'
            )
            return []
        except:
            ui_print(
                "[jackett] error: jackett couldnt be reached. Make sure your jackett base url is correctly formatted (default: http://localhost:9117)."
            )
            return []
        if not response.status_code == 200:
            if response.status_code in [401, 403]:
                ui_print(
                    ...
                    + ": it seems your api key is not working."
                )
            else:
                ui_print(
                    "[jackett] error "
                    + str(response.status_code)
                    + ": it seems jackett is reachable, but jackett returned an internal error."
                )
            return []
        try:
            response = json.loads(
            ...
            if regex.match(
                ...
                if not result.MagnetUri == None:
                    if not result.Tracker == None and not result.Size == None:
                        scraped_releases += [
                            releases.release(
                                "[jackett: " + str(result.Tracker) + "]",
                                "torrent",
                                result.Title,
                                [],
                                float(result.Size) / 1000000000,
                                [result.MagnetUri],
...
def resolve(result):
    ...
    try:
        link = session.get(
            result.Link, allow_redirects=False, timeout=float(resolver_timeout)
        )
        if "Location" in link.headers:
            if regex.search(
                r"(?<=btih:).*?(?=&)", str(link.headers["Location"]), regex.I
            ):
                if not result.Tracker == None and not result.Size == None:
                    scraped_releases += [
                        releases.release(
        ...
        ui_print(
            "[jackett] error: resolver couldnt get magnet/torrent for release: "
            + result.Title,
            ui_settings.debug,
        )
        return scraped_releases
