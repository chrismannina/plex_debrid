...
class media:
    ...
    def watch(self):
        ...
        if not self in media.ignore_queue:
            ...
            ui_print(
                "retrying download in 30min for item: "
                + self.query()
                + " - version/s ["
                + "],[".join(names)
                + "] - attempt "
                + str(self.ignored_count)
                + "/"
                + str(retries)
            )
        else:
    ...
    def download(self, retries=0, library=[], parentReleases=[]):
        ...
        i = 0
        self.Releases = []
        if self.type in ["movie", "show"] and (
            (not hasattr(self, "title") or self.title == "" or self.title == None)
            or (not hasattr(self, "year") or self.year == None or self.year == "")
        ):
            ui_print(
