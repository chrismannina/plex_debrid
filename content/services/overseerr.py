...
def setup(self):
    ...
    if len(plex.users) == 0 and len(trakt.users) == 0:
        print(
            "Looks like you havent connected plex_debrid to any other content service! Please setup at least one other content service."
        )
        time.sleep(3)
        return
    try:
        response = session.get(
            base_url + "/api/v1/request", headers={"X-Api-Key": api_key}, timeout=0.5
        )
    ...
    while not working_url:
        if base_url == "http://localhost:5055":
            print(
                "Looks like overseerr couldn't be reached under the default base url ('"
                + base_url
                + "')."
            )
        else:
            print(
                "Looks like overseerr couldn't be reached under the current base url ('"
                + base_url
                + "')."
            )
        print(
            "Please make sure overseerr is running and try again, or provide your overseerr base URL below."
        )
        print(
            "Please provide your overseerr base URL in the following format 'http://localhost:5055' or press enter to return to the main menu."
        )
        print()
    ...
    while not back:
        ...
        if choice in indices:
            if settings[int(choice) - 1].name == "Overseerr users":
                print()
                print(
                    "You can define which users approved requests should be downloaded by plex_debrid."
                )
                print()
...
def logerror(response):
    ...
    if response.status_code == 401:
        ui_print(
            "[overseerr] error: (401 unauthorized): overserr api key does not seem to work."
        )
...
class movie(classes.media):
    def __init__(self, other):
        ...
        self.watchlistedAt = datetime.datetime.timestamp(
        ...
        if len(users) > 0 and len(api_key) > 0:
            ...
            try:
                ...
                for element in response.results:
                    if (
                        not element in self.data
                        and (
                            element.requestedBy.displayName in users or users == ["all"]
                        )
                        and (
                            [str(element.media.status)] in allowed_movie_status
                            if element.type == "movie"
                            else [str(element.media.status)] in allowed_show_status
                        )
                    ):
                ...
                ui_print(
                    "[overseerr] error: overseerr couldnt be reached. turn on debug printing for more info."
                )
                last_requests = []
            ui_print("done")
            if last_requests == []:
            ...
            if len(sys.modules["content.services.plex"].users) > 0:
                ...
                ui_print(
                    "[overseerr] error: couldnt match overseerr content to either plex or trakt - add at least one plex or trakt user. No requests will be downloaded."
                )
                return
            ui_print(
                "[overseerr] matching overseerr requests to service "
                + matching_service
                + " ..."
            )
            add = []
            for element_ in last_requests:
                ...
                try:
                    ...
                    ui_print(
                        "[overseerr] error: couldnt match item to service "
                        + matching_service,
                        ui_settings.debug,
                    )
            for element in add:
        ...
        if len(users) > 0 and len(api_key) > 0:
            ...
            try:
                ...
                for element_ in response.results:
                    if (
                        not any(
                            x.id == element_.id and x.updatedAt == element_.updatedAt
                            for x in last_requests
                        )
                        and (
                            element_.requestedBy.displayName in users
                            or users == ["all"]
                        )
                        and (
                            [str(element_.media.status)] in allowed_movie_status
                            if element_.type == "movie"
                            else [str(element_.media.status)] in allowed_show_status
                        )
                    ):
                        ui_print(
                            '[overseerr] found new overseerr request by user "'
                        ...
                        if not element in self.data:
                            ...
                            if element.type == "show":
                                for season in element.Seasons:
                                    if not any(
                                        season.index == x.index
                                        for x in existing.Seasons
                                    ):
...
class library:
    ...
    class refresh(classes.refresh):
        ...
        def setup(cls, new=False):
            ...
            if len(api_key) == 0:
                print(
                    "It looks like you havent setup an overseerr api key. Please set up an overseerr api key first."
                )
                print()
