        ...
        while not back:
            ...
            if choice in indices:
                if settings[int(choice) - 1].name == "Trakt lists":
                    ...
                    if choice == "1":
                        print(
                            "Choose a trakt list that should be monitored by plex_debrid."
                        )
                        print()
                        i = 1
                        indices = []
                        ...
                        for user in users:
                            ...
                            if not response == None:
                                for p_list in response:
                                    if (
                                        not user[0] + "'s private list: " + p_list.name
                                        in lists
                                    ):
                                        print(
                                            str(i + 1)
                                        ...
                                        indices += [str(i + 1)]
                                        add_user += [
                                            user[0] + "'s private list: " + p_list.name
                                        ]
                                        i += 1
                        print()
                        ...
                        if choice == "0":
                            ...
                            try:
                                ...
                                if current_user[1] == "":
                                    print(
                                        "Please add at least one trakt user before adding a trakt list!"
                                    )
                                    time.sleep(5)
                                    return False
                            except:
...
def logerror(response):
    ...
    if response.status_code == 401:
        ui_print(
            "[trakt] error: (401 unauthorized): trakt api key for user '"
            + current_user[0]
                ...
                try:
                    ...
                    for element in watchlist_items:
                        if hasattr(element, "show"):
                            ...
                            try:
                                element.show.watchlistedAt = (
                                    datetime.datetime.timestamp(
                                        datetime.datetime.strptime(
                                            element.listed_at, "%Y-%m-%dT%H:%M:%S.000Z"
                                        )
                                    )
                                )
                            except:
                    ...
                    ui_print(
                        "[trakt error]: (exception): " + str(e), debug=ui_settings.debug
                    )
                    continue
            elif list_type == "collection":
                try:
                    watchlist_items, header = get(
                        "https://api.trakt.tv/sync/collection/shows?extended=full"
                    )
                    for element in watchlist_items:
                        if hasattr(element, "show"):
                            element.show.type = "show"
                ...
                try:
                    ...
                    if not p_list_id == None:
                        watchlist_items, header = get(
                            "https://api.trakt.tv/users/me/lists/"
                            + str(p_list_id)
                            + "/items/movies,shows?extended=full"
                        )
                        for element in watchlist_items:
                            if hasattr(element, "show"):
                                element.show.type = "show"
    ...
    def remove(self, original_element):
        ...
        if element.type == "tv":
            ...
            for attribute in element.__dict__.copy():
                if not (
                    attribute == "ids" or attribute == "title" or attribute == "year"
                ):
        ...
        if not deleted:
            ui_print(
                "[trakt error]: couldnt delete media item from any trakt list.",
        ...
        try:
            if hasattr(self, "first_aired"):
                if not self.first_aired == None:
                    self.originallyAvailableAt = datetime.datetime.strptime(
                        self.first_aired, "%Y-%m-%dT%H:%M:%S.000Z"
                    ).strftime("%Y-%m-%d")
                else:
                    self.originallyAvailableAt = datetime.datetime.utcnow().strftime(
                        "%Y-%m-%d"
                    )
            else:
                self.originallyAvailableAt = datetime.datetime.utcnow().strftime(
...
class library(classes.library):
    ...
    def __new__(self):
        ...
        ui_print(
            "[trakt] getting " + current_user[0] + "'s" + " entire trakt collection ..."
        )
        watchlist_items = []
        collection = []
        try:
            collection_movies, header = get(
                "https://api.trakt.tv/sync/collection/movies?extended=metadata"
            )
            collection_shows, header = get(
                "https://api.trakt.tv/sync/collection/shows?extended=metadata"
            )
    ...
    class refresh(classes.refresh):
        ...
        def __new__(cls, original_element):
            ...
            try:
                ...
                resolution = regex.search(
                    r"(2160|1080|720)(?=p)", element.Releases[0].title, regex.I
                )
            except:
                resolution = False
            if resolution:
            ...
            try:
                ...
                ui_print(
                    "[trakt] error: couldnt remove future episodes from show",
                    ui_settings.debug,
                )
                ...
                if hasattr(element, "Seasons"):
                    for season in element.Seasons:
                        ...
                        for attribute in season.__dict__.copy():
                            if not (
                                attribute == "ids"
                                or attribute == "episodes"
                                or attribute == "number"
                            ):
                                delattr(season, attribute)
                else:
                    ui_print(
                        "[trakt] error: couldnt find seasons in show object",
                        debug=ui_settings.debug,
                    )
    ...
    class ignore(classes.ignore):
        ...
        def setup(cls, new=False):
            ...
            if len(users) == 0:
                print(
                    "It looks like you havent setup a trakt user. Please set up a trakt user first."
                )
                print()
                for setting in settings:
                    if setting.name == "Trakt users":
            ...
            if not new:
                print(
                    "Current trakt user, whos watch status is used to ignore content: '"
                    + library.ignore.user
                    + "'"
                )
                print()
                ...
                if choice == "1":
                    ...
                    indices = []
                    print(
                        "Please choose a trakt users, whos watch status should be used to ignore content: "
                    )
                    print()
                    print("0) Back")
                    ...
                    if choice in indices:
                        ...
                        print()
                        print(
                            "Successfully changed to trakt user '"
                ...
                for index, plexuser in enumerate(users):
                    addable_users += [plexuser[0]]
                print(
                    "Please choose a trakt user, whos watch status should be used to ignore content: "
                )
                print()
                indices = []
                for index, ignoreuser in enumerate(addable_users):
        ...
        def add(self):
            ...
            try:
                ...
                if user == None:
                    print(
                        "[trakt] error: Could not find trakt ignore service user: '"
                        + ignoreuser
                        + "'. Make sure this trakt user exists."
                    )
                    return
                current_user = user
                ui_print(
                ...
                ui_print(
                    "trakt error: couldnt ignore item: " + str(e),
                    debug=ui_settings.debug,
                )
        ...
        def remove(self):
            ...
            try:
                ...
                ui_print(
                    "trakt error: couldnt un-ignore item: " + str(e),
                    debug=ui_settings.debug,
                )
        ...
        def check(self):
            ...
            try:
                ...
                if self.type == "show":
                    ...
                    for season in self.Seasons:
                        matching_season = next(
                            (x for x in match.Seasons if x == season), None
                        )
                ...
                ui_print(
                    "[trakt] error: couldnt check ignore status for item: " + str(e),
                    debug=ui_settings.debug,
                )
                return False
...
def aliases(self, lan):
    ...
    try:
        if len(users) > 0:
            ...
            response, header = get(
                "https://api.trakt.tv/" + type + "/" + str(self.ids.trakt) + "/aliases"
            )
            if not response == None:
                if len(response) > 0:
    ...
    try:
        if not lan == "en":
            if len(users) > 0:
                current_user = users[0]
                type = (
                    "shows" if self.type in ["show", "season", "episode"] else "movies"
                )
                response, header = get(
                    "https://api.trakt.tv/"
                    + type
...
def search(query, type):
    ...
    if type == "all":
        response, header = get(
            "https://api.trakt.tv/search/movie,show?query=" + str(query)
        )
    elif type == "movie":
