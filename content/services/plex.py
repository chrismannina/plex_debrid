...
def logerror(response):
    ...
    if response.status_code == 401:
        ...
        if name == "":
            ui_print(
                "plex error: (401 unauthorized): unnamed user token does not seem to work. check your plex user settings."
            )
        else:
            ui_print(
                "plex error: (401 unauthorized): token for user '"
                + name
                + "' does not seem to work. check your plex user settings."
            )
...
class watchlist(classes.watchlist):
        ...
        try:
            for user in users:
                ...
                while added < total:
                    total = 0
                    url = (
                        "https://metadata.provider.plex.tv/library/sections/watchlist/all?X-Plex-Container-Size=200&X-Plex-Container-Start="
                        + str(added)
                        + "&X-Plex-Token="
                        + user[1]
                    )
            ...
            try:
                ...
                ui_print(
                    "[plex error]: (watchlist exception): could not sort watchlist chronologically for unknown reason",
                    debug=ui_settings.debug,
                )
        except Exception as e:
            ui_print("done")
            ui_print(
                "[plex error]: (watchlist exception): " + str(e),
                debug=ui_settings.debug,
            )
        ...
        if hasattr(item, "user"):
            if isinstance(item.user[0], list):
                for user in item.user:
                    url = (
                        "https://metadata.provider.plex.tv/actions/removeFromWatchlist?ratingKey="
                        + item.ratingKey
                        + "&X-Plex-Token="
                        + user[1]
                    )
    ...
    def add(self, item, user):
        ui_print(
            '[plex] item: "' + item.title + '" added to ' + user[0] + "`s watchlist"
        )
        url = (
            "https://metadata.provider.plex.tv/actions/addToWatchlist?ratingKey="
            + item.ratingKey
            + "&X-Plex-Token="
            + user[1]
        )
    ...
    def __init__(self, other):
        ...
        while len(self.Episodes) < self.leafCount:
            url = (
                ...
                + self.ratingKey
                + "/children?includeUserState=1&X-Plex-Container-Size=200&X-Plex-Container-Start="
                + str(len(self.Episodes))
                + "&X-Plex-Token="
                + token
            )
            response = get(url)
                ...
                if hasattr(response, "MediaContainer"):
                    ...
                    if hasattr(response.MediaContainer, "Metadata"):
                        for episode_ in response.MediaContainer.Metadata:
                            ...
                            viewCount += (
                                1
                                if hasattr(episode_, "viewCount")
                                and episode_.viewCount > 0
                                else 0
                            )
    ...
    def __init__(self, ratingKey):
        ...
        while not success:
            url = (
                "https://metadata.provider.plex.tv/library/metadata/"
                + ratingKey
                + "?includeUserState=1&X-Plex-Token="
                + token
            )
            response = get(url)
            if not response == None:
                ...
                url = (
                    "https://metadata.provider.plex.tv/library/metadata/"
                    + ratingKey
                    + "/children?includeUserState=1&X-Plex-Container-Size=200&X-Plex-Container-Start=0&X-Plex-Token="
                    + token
                )
                response = get(url)
                if not response == None:
                    if hasattr(response, "MediaContainer"):
                        if hasattr(response.MediaContainer, "Metadata"):
                            ...
                            for index, Season in enumerate(
                                ...
                                t = Thread(
                                    target=multi_init,
                                    args=(season, Season, results, index),
                                )
                                threads.append(t)
                                t.start()
...
class library(classes.library):
    ...
    class refresh(classes.refresh):
        ...
        def setup(cls, new=False):
            ...
            if len(users) == 0:
                print(
                    "It looks like you havent setup a plex user. Please set up a plex user first."
                )
                print()
                for setting in settings:
                    if setting.name == "Plex users":
            ...
            while not working:
                try:
                    response = get(
                        library.url + "/library/sections/?X-Plex-Token=" + users[0][1]
                    )
                    working = True
                    if len(response.MediaContainer.Directory) == 0:
                        print(
                            "It looks like this server does not have any libraries set-up! Please open the plex webui, setup at least one library and point it to your mounted debrid service drive."
                        )
                        print()
                    ...
                    working = False
                    print(
                        "It looks like your plex server could not be reached at '"
                        + library.url
                        + "'. Make sure that plex media server is running, that you have claimed the server and that you have created at least one plex library."
                    )
                    print()
                    for setting in settings:
                        if setting.name == "Plex server address":
            ...
            if not new:
                ...
                while not back:
                    ...
                    if choice0 == "1":
                        print(
                            "Current plex library sections that are refreshed after a successful download: "
                        )
                        print()
                        print("0) Back")
                        indices = []
                        ...
                        print()
                        print(
                            "Type 'add' to add another plex library section that should be refreshed."
                        )
                        print()
                        ...
                        if choice in indices:
                            ...
                            if choice2 == "1":
                                library.refresh.sections.remove(
                                    library.refresh.sections[int(choice) - 1]
                                )
                        if choice == "0":
                            back = True
                        if choice == "add":
                            ...
                            if len(sections) == 0:
                                print()
                                print(
                                    "It seems youve added all plex library sections of this server!"
                                )
                                time.sleep(3)
                                return
                            indices = []
                            print(
                                "Please choose a plex library section that should be refreshed after a successful download: "
                            )
                            print()
                            print("0) Back")
                            ...
                            choice = input("Please choose a plex library section: ")
                            if choice in indices:
                                library.refresh.sections += [
                                    sections[int(choice) - 1].key
                                ]
                                print()
                                print(
                                    "Successfully added plex library section '"
                                    + sections[int(choice) - 1].title
                                    + "'"
                                )
                                print()
                                time.sleep(3)
                ...
                while not back:
                    ...
                    if len(sections) == 0:
                        ...
                        return
                    print(
                        "Please choose at least one plex library section that should be refreshed after a successful download: "
                    )
                    print()
                    indices = []
                ...
                for path in paths:
                    ...
                    if library.refresh.partial == "true":
                        ...
                        url = (
                            ...
                            + "/library/sections/"
                            + section
                            + "/refresh?X-Plex-Token="
                            + users[0][1]
                        )
                        ui_print(
                            "refreshing plex via url: " + url, debug=ui_settings.debug
                        )
                        response = session.get(url)
            except Exception as e:
        ...
        def __new__(cls, element):
            try:
                names = []
                element_type = (
                    "show" if element.type in ["show", "season", "episode"] else "movie"
                )
                ...
                for section_ in response.MediaContainer.Directory:
                    if (
                        section_.key in library.refresh.sections
                        and element_type == section_.type
                    ):
                        names += [section_.title]
                        folders = []
                        for location in section_.Location:
                            if (
                                hasattr(element, "downloaded_releases")
                                and len(element.downloaded_releases) > 0
                                and library.refresh.partial == "true"
                            ):
                ...
                try:
                    ...
                    ui_print(
                        "[plex] error: provided refresh delay is not a number! using default 2 second delay."
                    )
                time.sleep(delay)
                ui_print(
                    "[plex] refreshing "
                ...
                results = [None]
                t = Thread(
                    target=multi_init, args=(library.refresh.call, paths, results, 0)
                )
                t.start()
            except:
                ui_print(
                    "[plex] error: couldnt refresh libraries. Make sure you have setup a plex user!"
                )
        ...
        def call(element):
            try:
                ...
                if library_item == None:
                    ui_print(
                        '[plex] error: couldnt add lables - item: "'
                        + element.query()
                ...
                for tag in tags:
                    ...
                    url = (
                        library.url
                        + "/library/sections/"
                        + str(library_item.librarySectionID)
                        + "/all?type="
                        + type_string
                        + "&id="
                        + library_item.ratingKey
                        + "&label.locked=1"
                        + tags_string
                        + "&X-Plex-Token="
                        + users[0][1]
                ...
                response = get(url)
                library_item.__dict__.update(
                    response.MediaContainer.Metadata[0].__dict__
                )
            except Exception as e:
                ui_print(
                    "[plex] error: couldnt add lables! Turn on debug printing for more info."
                )
                ui_print(str(e), debug=ui_settings.debug)
        ...
        def __new__(cls, element):
            ...
            try:
                ...
                for version in element.downloaded_versions:
                    if (
                        element.query() in version
                        and not "Version: " + version.split("[")[-1][:-1] in tags
                    ):
                ...
                results = [None]
                t = Thread(
                    target=multi_init, args=(library.lable.call, element, results, 0)
                )
                t.start()
            except Exception as e:
                ui_print(
    ...
    class ignore(classes.ignore):
        ...
        def setup(cls, new=False):
            ...
            if not library.name in classes.library.active:
                print(
                    "It looks like you havent set your library collection service to plex. This is needed to accurately match content to the plex media type."
                )
            ...
            if not new:
                print(
                    "Current plex user, whose plex discover watch status is used to ignore content: '"
                    + library.ignore.user
                    + "'"
                )
                print()
                ...
                if choice == "1":
                    ...
                    indices = []
                    print(
                        "Please choose a plex users, whos plex discover watch status should be used to ignore content: "
                    )
                    print()
                    print("0) Back")
                    ...
                    if choice in indices:
                        ...
                        print()
                        print(
                            "Successfully changed to plex user '"
                            + addable_users[int(choice) - 1]
                            + "'"
                        )
                        print()
                        time.sleep(3)
                ...
                for index, plexuser in enumerate(users):
                    addable_users += [plexuser[0]]
                print(
                    "Please choose a plex user, whos plex discover watch status should be used to ignore content: "
                )
                print()
                indices = []
                for index, ignoreuser in enumerate(addable_users):
        ...
        def add(self):
            try:
                ...
                if user == None:
                    print(
                        "[plex] error: Could not find plex ignore service user: '"
                        + ignoreuser
                        + "'. Make sure this plex user exists."
                    )
                    return
                ui_print(
                    "[plex] ignoring item: "
                ...
                url = (
                    "https://metadata.provider.plex.tv/actions/scrobble?identifier=tv.plex.provider.metadata&key="
                    + self.ratingKey
                    + "&X-Plex-Token="
                    + user[1]
                )
                ...
                ui_print(
                    "plex error: couldnt ignore item: " + str(e),
                    debug=ui_settings.debug,
                )
        ...
        def remove(self):
            try:
                ...
                url = (
                    "https://metadata.provider.plex.tv/actions/unscrobble?identifier=tv.plex.provider.metadata&key="
                    + self.ratingKey
                    + "&X-Plex-Token="
                    + user[1]
                )
                ...
                ui_print(
                    "plex error: couldnt un-ignore item: " + str(e),
                    debug=ui_settings.debug,
                )
        ...
        def check(self):
            try:
                ...
                ui_print(
                    "[plex] error: couldnt check ignore status for item: " + str(e),
                    debug=ui_settings.debug,
                )
                return False
    ...
    def __new__(self, silent=False):
        ...
        try:
            ...
            for Directory in response.MediaContainer.Directory:
                if (
                    [Directory.key] in library.check or library.check == []
                ) and Directory.type in ["movie", "show"]:
                    types = ["1"] if Directory.type == "movie" else ["2", "3", "4"]
                    sections += [[Directory.key, types]]
                    names += [Directory.title]
        except:
            ui_print(
                "[plex error]: couldnt reach local plex server at: "
                + library.url
                + " to determine library sections. Make sure the address is correct, the server is running, and youve set up at least one library."
            )
        if len(sections) == 0:
            return list_
        if not silent:
            ui_print(
                '[plex] getting plex library section/s "' + '","'.join(names) + '" ...'
            )
        for section, types in sections:
            ...
            if len(section_response) == 0:
                ui_print(
                    "[plex error]: couldnt reach local plex library section '"
                    + section
                    + "' at server address: "
                    + library.url
        ...
        if len(list_) - len(current_library) > 0:
            ui_print("done")
            ui_print(
                "[plex] getting metadata for "
                + str(len(list_) - len(current_library))
        ...
        for item in list_:
            try:
                ...
                ui_print("done")
                ui_print(
                    "[plex error]: found incorrectly matched library item : "
                    + item.title
                    + " - this item needs a metadata refresh (open plex webui, find item, open item menu, refresh metadata)."
                )
        ui_print("done")
...
def search(query, library=[]):
    ...
    url = (
        "https://metadata.provider.plex.tv/library/search?query="
        + query
        + "&limit=20&searchTypes=movies%2Ctv&includeMetadata=1&X-Plex-Token="
        + users[0][1]
    )
    response = get(url)
    try:
...
def match(self):
    ...
    if some_local_media == None:
        ui_print(
            "[plex error]: couldnt match content to plex media type, no media of the same type found!"
        )
        return None
    if self.type == "movie":
    ...
    for EID in self.EID:
        ...
        url = (
            current_module.library.url
            + "/library/metadata/"
            + some_local_media.ratingKey
            + "/matches?manual=1&title="
            + query
            + "&agent="
            + agent
            + "&language=en-US&X-Plex-Token="
            + users[0][1]
        )
        response = get(url)
        try:
