...
def ignored():
    ...
    while not back:
        ...
        print()
        choice = input(
            "Choose a media item that you want to remove from the ignored list: "
        )
        if choice in indices:
            print(
                "Media item: "
                + content.classes.ignore.ignored[int(choice) - 1].query()
...
def scrape():
    ...
    if choice in indices and not choice == str(index + 2):
        obj.version = releases.sort.version(
            releases.sort.versions[int(choice) - 1][0],
...
def setup():
    if os.path.exists(config_dir + "/settings.json"):
        if os.path.getsize(config_dir + "/settings.json") > 0 and os.path.isfile(
    ...
    try:
        ...
        print(
            "Error: It seems this terminal is not interactive! Please make sure to allow user input in this terminal. For docker, add the 'interactive' flag ('-ti')."
        )
        time.sleep(10)
        exit()
...
def save(doprint=True):
    ...
    try:
        ...
        print()
        print(
            "Error: It looks like plex_debrid can not write your settings into a config file. Make sure you are running the script with write or administator privilege."
        )
        print()
...
def load(doprint=False, updated=False):
    ...
    if "version" not in settings:
        ...
        not settings["version"][0] == ui_settings.version[0]
        and not ui_settings.version[2] == []
    ):
        update(settings, ui_settings.version)
        updated = True
    ...
    if "Library Service" in settings:
        ...
        if settings["Library Service"] == ["Plex Library"]:
            if (
                'Plex "movies" library' in settings
                and 'Plex "shows" library' in settings
            ):
                settings["Plex library refresh"] = [
                    settings['Plex "movies" library'],
                    settings['Plex "shows" library'],
                ]
    ...
    for category, load_settings in settings_list:
        for setting in load_settings:
            if (
                setting.name in settings
                and not setting.name == "version"
                and not setting.name == "Content Services"
            ):
...
def update_available():
    try:
        response = requests.get(
            "https://raw.githubusercontent.com/itsToggle/plex_debrid/main/ui/ui_settings.py",
            timeout=0.25,
        )
...
def update(settings, version):
    ...
    print(
        "There has been an update to plex_debrid, which is not compatible with your current settings:"
    )
    print()
    print(version[1])
    print()
...
def threaded(stop):
    ...
    try:
        ...
        ui_print(
            "couldnt sort monitored media by newest, using default order.",
            ui_settings.debug,
        )
    if len(library) > 0:
            ...
            if hasattr(element, "download"):
                ...
                if t1 - t0 >= 5:
                    if (
                        ...
                        or overseerr_requests.update()
                        or trakt_watchlist.update()
                    ):
                        library = content.classes.library()[0]()
                        if len(library) == 0:
                            continue
                        new_watchlists = (
                            plex_watchlist + trakt_watchlist + overseerr_requests
                        )
                        try:
                ...
                if hasattr(element, "download"):
                    ...
                    if element.type == "show":
                        for season in element.Seasons:
                            if (
                                season in content.classes.media.ignore_queue
                                or not newly_added
                            ):
                                newly_added = False
                                break
