                    ...
                    if self.subclass:
                        ...
                        while not back:
                            ...
                            for service in self.cls.__subclasses__():
                                if (
                                    not service.name in getattr(self.cls, self.key)
                                    and not "(NOT FUNCTIONAL)" in service.name
                                ):
        ...
        if isinstance(getattr(self.cls, self.key), list):
            ...
            while not working:
                if self.subclass:
                    ...
                    for service in self.cls.__subclasses__():
                        if (
                            not "(NOT FUNCTIONAL)" in service.name
                            and not service.name == "Overseerr"
                        ):
                            print(str(index + 1) + ") " + service.name)
                            indices += [str(index + 1)]
                            services += [service]
                    ...
                    if self.name == "Plex users":
                        ...
                        response = content.services.plex.session.get(
                            url, headers=content.services.plex.headers
                        )
                        if response.status_code == 200:
                            ...
                            print()
                            print(
                                "Looks like this plex token does not work. Please enter a valid token."
                            )
                            print()
                    else:
                        working = True
...
settings_list = [
    [
        ...
        [
            ...
            setting(
                "Plex users",
                [
                    "Please provide a name for this Plex user: ",
                    "Please provide the Plex token for this Plex user: ",
                ],
            ...
            setting(
                ...
                content.services.plex.watchlist,
                "autoremove",
                hidden=True,
            ...
            setting(
                "Trakt users",
                [
                    "Please provide a name for this Trakt user: ",
            ...
            setting(
                "Trakt auto remove",
                'Please choose which media type/s should be removed from your watchlist after successful download ("movie","show","both" or "none"): ',
                content.services.trakt.watchlist,
                "autoremove",
                hidden=True,
                help="By default, movies are removed from your watchlist after a successful download. In this setting you can choose to automatically remove shows, shows and movies or nothing.",
            ),
            setting(
                "Trakt early movie releases",
                'Please enter "true" or "false": ',
                content.services.trakt,
                "early_releases",
                help="plex_debrid can check for early releases of movies by checking trakt for 'latest releases' lists. You can turn this feature on or off.",
                hidden=True,
            ),
            setting(
                "Overseerr users",
                ...
                "users",
                entry="user",
                help="Please specify which users requests should be downloaded by plex_debrid.",
                hidden=True,
            ),
            setting(
                "Overseerr API Key",
                "Please specify your Overseerr API Key: ",
                content.services.overseerr,
                "api_key",
                hidden=True,
            ),
            setting(
                "Overseerr Base URL",
                "Please specify your Overseerr base URL: ",
                content.services.overseerr,
                "base_url",
                hidden=True,
            ),
    ...
    [
        ...
        [
            ...
            setting(
                "Trakt library user",
                [""],
                content.services.trakt.library,
                "user",
                hidden=True,
            ),
            setting(
                "Trakt refresh user",
                [""],
                content.services.trakt.library.refresh,
                "user",
                hidden=True,
            ),
            setting(
                ...
                [""],
                content.services.plex.library.refresh,
                "sections",
                hidden=True,
                moveable=False,
            ),
            setting(
                "Plex library partial scan",
                'Please enter "true" or "false": ',
                content.services.plex.library.refresh,
                "partial",
                hidden=True,
                help="Specify wether or not plex_debrid should attempt to partially scan your plex libraries.",
            ),
            setting(
                "Plex library refresh delay",
                "Please enter a number (e.g 420 or 69.69): ",
                content.services.plex.library.refresh,
                "delay",
                hidden=True,
                help="Specify the amount of seconds plex_debrid should wait between adding a torrent and scanning your plex libraries.",
            ),
            setting(
                "Plex server address",
                "Please enter your Plex server address: ",
                content.services.plex.library,
                "url",
                hidden=True,
            ),
            setting(
            ...
            setting(
                "Plex ignore user",
                "",
                content.services.plex.library.ignore,
                "user",
                hidden=True,
            ),
            setting(
                "Trakt ignore user",
                "",
                content.services.trakt.library.ignore,
                "user",
                hidden=True,
            ),
            setting(
                "Local ignore list path",
                "Please provide a path where the list ignored media items should be saved: ",
                content.services.textfile.library.ignore,
                "path",
                hidden=True,
            ),
            setting(
    ...
    [
        ...
        [
            ...
            setting(
                "Rarbg API Key",
                "The Rarbg API Key gets refreshed automatically, enter the default value: ",
            ...
            setting(
                "Jackett Base URL",
                "Please specify your Jackett base URL: ",
            ...
            setting(
                "Jackett API Key",
                "Please specify your Jackett API Key: ",
                scraper.services.jackett,
                "api_key",
                hidden=True,
            ),
            setting(
                "Jackett resolver timeout",
                "Please enter the resolver timeout in seconds: ",
                scraper.services.jackett,
                "resolver_timeout",
                hidden=True,
            ),
            setting(
                "Jackett indexer filter",
                'Please enter the jackett indexer filters that should be used, seperated by a "," character. Enter "all" to not filter your indexers: ',
                scraper.services.jackett,
                "filter",
                hidden=True,
            ),
            setting(
                "Prowlarr Base URL",
                "Please specify your Prowlarr base URL: ",
            ...
            setting(
                "Prowlarr API Key",
                "Please specify your Prowlarr API Key: ",
                scraper.services.prowlarr,
                "api_key",
                hidden=True,
            ),
            setting(
                "Orionoid API Key",
                'Please open your favorite browser, log into your orionoid account and open "https://auth.orionoid.com". Enter this code: ',
                scraper.services.orionoid,
                "token",
                hidden=True,
                oauth=True,
            ),
            setting(
                ...
                [
                    "Please enter a valid orionoid parameter: ",
                    "Please enter a valid value: ",
                ],
                scraper.services.orionoid,
                "default_opts",
                entry="parameter",
                help='This settings lets you control the orionoid scraping parameters. Check out the possible parameters and their values at "https://panel.orionoid.com/" in the "Developers" menu, section "API Docs" under "Stream API".',
                hidden=True,
            ),
            setting(
                "Nyaa parameters",
                'Enter custom url parameters. Categories: anime "&c=1_0", anime raw "&c=1_4", anime EN subs "&c=1_2", anime non-EN subs "&c=1_3". Enter your parameters (default: "&c=1_0&s=seeders&o=desc"): ',
            ...
            setting(
                "Nyaa sleep time",
                'Enter a time in seconds to sleep between requests (default: "5"): ',
                scraper.services.nyaa,
                "sleep",
                hidden=True,
            ),
            setting(
                "Nyaa proxy",
                'Enter a proxy to use for nyaa (default: "nyaa.si"): ',
                scraper.services.nyaa,
                "proxy",
                hidden=True,
            ),
